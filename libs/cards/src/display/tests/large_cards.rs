...
fn test_large_cards_dimensions() {
    ...
    for card in deck.cards.iter() {
        ...
        assert_eq!(lines.len(), EXPECTED_HEIGHT, 
            ...
            card, EXPECTED_HEIGHT, lines.len());
...
fn test_large_cards_borders() {
    ...
    for card in deck.cards.iter() {
        ...
        assert_eq!(top_border.chars().nth(0).unwrap(), '╭', 
            "Card {:?} top-left corner is incorrect", card);
        assert_eq!(top_border.chars().last().unwrap(), '╮', 
            "Card {:?} top-right corner is incorrect", card);
        assert!(top_border.chars().skip(1).take(11).all(|c| c == '─'), 
            "Card {:?} top border line is incorrect", card);
        
        // 2. Ensure the bottom border has the correct corners and lines
        let bottom_border = lines[lines.len() - 1];
        assert_eq!(bottom_border.chars().nth(0).unwrap(), '╰', 
            "Card {:?} bottom-left corner is incorrect", card);
        assert_eq!(bottom_border.chars().last().unwrap(), '╯', 
            "Card {:?} bottom-right corner is incorrect", card);
        assert!(bottom_border.chars().skip(1).take(11).all(|c| c == '─'), 
            "Card {:?} bottom border line is incorrect", card);
        ...
        for i in 1..(lines.len() - 1) {
            ...
            assert_eq!(line.chars().nth(0).unwrap(), '│', 
                "Card {:?} line {} does not start with vertical bar", card, i);
            assert_eq!(line.chars().last().unwrap(), '│', 
                "Card {:?} line {} does not end with vertical bar", card, i);
        }
    }
}
    ...
    for card in deck.cards.iter() {
        ...
        if let Some(suit) = card.suit {
            ...
            if card.rank == Rank::Joker {
                ...
                assert!(display.contains("JOKER"), "Joker card should contain 'JOKER' text");
            }
        }
    }
}